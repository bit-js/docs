...
- If a function has no parameter then do not pass in the context as an argument.
- If a function is not asynchronous then remove `await`.
- If no functions have parameters then the `run` function does not need the context as a parameter.
- If no functions are asynchronous then the `run` function can be synchronous.

The routing phase is JIT compiled with the [Blitz](https://www.npmjs.com/package/@bit-js/blitz) router.
You can ignore JIT compilation for this phase by using [EdgeRouter](https://www.npmjs.com/package/@bit-js/blitz#edge-router).
```ts
